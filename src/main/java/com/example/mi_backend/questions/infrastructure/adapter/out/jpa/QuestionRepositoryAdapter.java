package com.example.mi_backend.questions.infrastructure.adapter.out.jpa;
...
import com.example.mi_backend.questions.application.port.out.*;
import com.example.mi_backend.questions.domain.model.Question;
import com.example.mi_backend.questions.infrastructure.adapter.out.jpa.entity.*;
import com.example.mi_backend.questions.infrastructure.adapter.out.jpa.mapper.QuestionJpaMapper;
import com.example.mi_backend.questions.infrastructure.adapter.out.jpa.repository.*;
...
public class QuestionRepositoryAdapter implements
    ...
    @Override
    public List<com.example.mi_backend.questions.domain.model.MatchingItem> loadLeftItems(Long questionId) {
        return leftRepo.findByQuestionIdOrderByPositionAsc(questionId)
                .stream().map(le -> com.example.mi_backend.questions.domain.model.MatchingItem
                        .rehydrate(le.getId(), le.getText()))
                .toList();
    }

    @Override
    public List<com.example.mi_backend.questions.domain.model.MatchingItem> loadRightItems(Long questionId) {
        return rightRepo.findByQuestionIdOrderByPositionAsc(questionId)
                .stream().map(re -> com.example.mi_backend.questions.domain.model.MatchingItem
