package com.example.mi_backend.forms.application.usecase;

import com.example.mi_backend.forms.application.port.in.SetAccessModeUseCase;
import com.example.mi_backend.forms.application.port.out.LoadFormPort;
import com.example.mi_backend.forms.application.port.out.SaveFormPort;
import com.example.mi_backend.forms.domain.exception.FormNotFoundException;
import com.example.mi_backend.forms.domain.model.AccessMode;
import com.example.mi_backend.forms.domain.model.Form;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.time.Clock;
import java.time.Instant;

@Service
@Transactional
public class SetAccessModeService implements SetAccessModeUseCase {
    private final LoadFormPort loadPort; private final SaveFormPort savePort; private final Clock clock;
    public SetAccessModeService(LoadFormPort loadPort, SaveFormPort savePort, Clock clock){
        this.loadPort = loadPort; this.savePort = savePort; this.clock = clock;
    }
    @Override public Form handle(Long formId, AccessMode mode){
        var form = loadPort.loadById(formId).orElseThrow(() -> new FormNotFoundException(formId));
        var updated = form.setAccessMode(mode, Instant.now(clock));
        return savePort.save(updated);
    }
}
